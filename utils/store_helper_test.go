...
import (
	...
	"github.com/osmosis-labs/osmosis/v11/utils"
)
...
func (s *TestSuite) TestGatherValuesFromIteratorWithStop() {
	...
	for name, tc := range testcases {
		s.Run(name, func() {
			...
			actualValues, err := utils.GatherValuesFromIteratorWithStop(iterator, mockParseValueFn, mockStop)
...
func (s *TestSuite) TestNoStopFn_AlwaysFalse() {
	s.Require().False(utils.NoStopFn([]byte(keyA)))
	s.Require().False(utils.NoStopFn([]byte(keyB)))
}
